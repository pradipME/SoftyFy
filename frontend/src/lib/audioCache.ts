/**
 * Shared cache-layout constants used by the Service Worker (sw.ts) and the
 * player's preloader (PlayerContext.tsx). Keeping them in one module means a
 * rename can never silently desync the service worker from the page.
 */
